...
<div class="camera-page">
    ...
    <section class="camera-summary-grid">
        ...
        <article class="camera-summary-card">
            ...
            <strong style="color: var(--accent)">{{ $stats['active'] }}</strong>
            <span>Currently included in worker sync.</span>
        </article>
        <article class="camera-summary-card">
            <small>Network Feeds</small>
            <strong style="color: var(--accent3)">{{ $stats['network'] }}</strong>
            <span>RTSP, HTTP, and IP camera sources.</span>
        </article>
        <article class="camera-summary-card">
            <small>USB Cameras</small>
            <strong style="color: var(--warn)">{{ $stats['usb'] }}</strong>
    ...
    <section class="camera-panel">
        ...
        <div class="camera-panel__body">
            <form method="POST" action="{{ route('cameras.store') }}" class="camera-config-form">
                @csrf
                @include('cameras.partials.form-fields', [
                    'camera' => null,
                    'settings' => [
    ...
    <section class="camera-panel">
        <div class="camera-panel__header">
            ...
            <div class="camera-chip-row">
                <span class="camera-chip">{{ $stats['total'] }} Total</span>
                <span class="camera-chip">{{ $stats['active'] }} Active</span>
            </div>
        </div>
        <div class="camera-panel__body">
            @if($cameras->isEmpty())
                ...
                <article class="camera-record">
                    <div class="camera-record__header">
                        <div>
                            <div class="camera-record__title">
                                ...
                                <span class="camera-chip">Camera #{{ $camera->id }}</span>
                                <span class="camera-chip">{{ ucfirst($camera->vendor ?: 'generic') }}</span>
                            </div>
                            <div class="camera-record__meta">
                                Last seen: {{ $camera->last_seen_at?->format('Y-m-d H:i:s') ?? 'Never' }}
                            </div>
                        </div>
                        <span class="badge-surv {{ $camera->status === 'active' ? 'bg-accent3' : 'bg-secondary' }}">
                            {{ strtoupper($camera->status) }}
                        </span>
                    </div>

                    <div class="camera-record__body">
                        <form id="camera-update-{{ $camera->id }}" method="POST" action="{{ route('cameras.update', $camera) }}" class="camera-config-form">
                            @csrf
                            @method('PUT')
                            @include('cameras.partials.form-fields', [
                                'camera' => $camera,
                    ...
                    <div class="camera-record__footer">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                            <button type="submit" form="camera-update-{{ $camera->id }}" class="btn btn-sm btn-accent text-dark fw-bold px-3">
                                <i class="fas fa-floppy-disk me-2"></i> Save Changes
                            </button>
                            <form method="POST" action="{{ route('cameras.destroy', $camera) }}" onsubmit="return confirm('Remove this camera from MetroNet?');">
                                @csrf
                                @method('DELETE')
...
<template id="vendor-profiles-json">@json($vendorOptions)</template>
